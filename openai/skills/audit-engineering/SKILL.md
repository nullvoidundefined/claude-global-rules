...
---
<!-- GENERATED by openai/build.mjs from agents/audit-engineering.md. Do not edit; change the source and run: node openai/build.mjs --write -->

# audit-engineering

Use this agent to conduct a comprehensive engineering audit (CTO perspective) of a codebase: architecture, code quality, security, database, API design, performance, testing, dependencies, deployment, bug-fix discipline, runbook-vs-code drift, and workspace hygiene. Use when the user asks for an engineering audit, before a major launch, or after shipping 5 or more commits on a single surface. Produces `docs/audits/YYYY-MM-DD-engineering.md` and commits it. Scope can be narrowed in the dispatch prompt (e.g., "focus on server/" or "only the generation pipeline").

Delegate this to the `audit-engineering` custom agent. If custom agents are unavailable in this build, read `~/.claude/agents/audit-engineering.md` and carry out that role definition in this conversation, honoring its model-routing and output-discipline sections.
