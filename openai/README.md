...
The same rules, gates, skills, audit roles, and session procedures as the Claude Code configuration, in the shapes OpenAI Codex reads: the CLI, the IDE extension, and the cloud agent that ChatGPT runs all load `~/.codex/AGENTS.md`, `~/.codex/skills/`, `~/.codex/agents/`, and `~/.codex/hooks.json`. ChatGPT's chat interface itself reads no files; this port is for the Codex agent it delegates to.

Everything except this README, `build.mjs`, `install.sh`, and the adapter is generated from the canonical sources one directory up, and `enforce/tests/openai-port.test.sh` fails the suite when the generated tree is stale.
...
That symlinks `~/.codex/AGENTS.md`, `~/.codex/hooks.json`, `~/.codex/skills`, and `~/.codex/agents` into this directory, so a `git pull` of `~/.claude` updates Codex too. It refuses to replace anything at those paths it did not create and prints the `mv` to run instead. Then, in a Codex session, `/hooks` lists the hook entries for you to trust (Codex trusts hooks by content hash, so every change to a hook script or to `hooks.json` needs a fresh trust), and `/skills` shows the skills. The repo has to live at `~/.claude`: `AGENTS.md` points at `~/.claude/...` for every on-demand read and `hooks.json` runs the adapter from `~/.claude/openai/hooks/`.
...
`--check` reports drift and is what the fixture test runs. Each Claude Code-specific sentence is rewritten through a substitution whose needle must still exist in the source, so a source edit that moves it fails the build rather than emitting stale prose. `AGENTS.md` is also size-checked: Codex loads every AGENTS.md in one 32 KiB budget (`project_doc_max_bytes`), so the global file stays under 24 KiB to leave room for project files.
...
| `settings.json` hooks | `hooks.json` + `hooks/codex-hook-adapter.sh` | Generated from `settings.json`: same schema, same events, same matchers. See `PORT-STATUS.md` for the per-hook table and the three that have no Codex event |
...
Codex implemented its hooks on Claude Code's protocol: `hooks.json` has the same shape, `PreToolUse` sees `tool_name`, `tool_input`, and `cwd`, a hook denies with `hookSpecificOutput.permissionDecision`, and `Stop` blocks with `decision: block`. So `openai/hooks.json` is `settings.json`'s hook block with three edits, and `hooks/codex-hook-adapter.sh` is the only Codex-specific script:
