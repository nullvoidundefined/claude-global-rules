...
fired: R-001 no-em-dash.sh blocked an Edit; replaced with colon
miss: R-101 leaked a value via sed; gap: codify compare-in-shell
EOF
...
printf '# Rule fires log\n\nheader\n\n2020-01-01 R-001 no-em-dash.sh blocked an Edit; replaced with colon\n' > "$FIRES"
...
check "fire entry was written"  has_line "$FIRES"  "R-001 no-em-dash.sh blocked an Edit"
check "miss entry was written"  has_line "$MISSES" "R-101 MISS leaked a value via sed"
...
check "miss not duplicated on re-run" count_is "$MISSES" "R-101 MISS leaked a value via sed" 1
