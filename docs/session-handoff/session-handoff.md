...
1. Merge the branch, pull, then `node ~/.claude/cursor/build.mjs --write` and `node ~/.claude/openai/build.mjs --write`. The Codex build refuses to replace an existing `~/.codex/AGENTS.md`; move it aside first or pass `--force`.
