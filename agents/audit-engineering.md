...
- **Performance**: N+1 queries, bundle size, caching strategy, cold start, and **client-side request frequency**. Grep the client tree for `refetchInterval`, `setInterval`, `useInterval`, and any custom polling helper. For every match, cross-check (1) the endpoint's per-call cost (DB aggregate, external API, slow handler), and (2) the data freshness model. If the endpoint's value only changes on a discrete event (an agent turn completes, a payment webhook fires, a job finishes), a forever-running poll is wasted load and should be event-driven invalidation (`queryClient.invalidateQueries`, WebSocket push, SSE event). A polling interval without an `enabled:` gate or a stop condition is a P1 finding. Motivation: 2026-05-29, the voyager `/trips/:id/costs` endpoint was polled every 5 seconds while ChatBox was mounted (`refetchInterval: 5000`) against an `agent_turn_cost` SUM aggregate that only changes once per agent turn. The audit did not catch it because the rubric only listed server-side perf concerns. The user observed it in production logs. Code-only audits miss this class; the auditor must combine a grep for polling primitives with an honest read of endpoint cost and event semantics.
