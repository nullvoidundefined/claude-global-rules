...
**Why:** A project meant to teach vector databases was built as a recommender whose real queries were reasoning/intent tasks (identify-an-item-from-a-fuzzy-description, compositional multi-constraint natural-language requests). Those are LLM strengths and embedding weaknesses, so vector search was the wrong primary tool and lost to a plain LLM call. The mismatch surfaced only after weeks of work, and the user concluded they had been building under a false pretense. The miss was not flagging the technology-vs-task mismatch at kickoff.
