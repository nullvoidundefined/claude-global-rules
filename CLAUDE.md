...
The canonical rule file, loaded into every Claude Code session at startup. Rule IDs are stable; new rules append, retired rules are removed and left as a one-line tombstone. Document order is by importance, not by ID; the ID is a stable label. See `PROTOCOL.md` for the ten-layer framework these rules implement.
...
R-225: Each project is an independent git repo. No cross-project or cross-category source imports via relative paths; `production/`, `development/`, `templates/`, and `tools/` never reach into each other. Shared code is published from the `tools/nullvoidundefined-packages` pnpm workspace and consumed as a versioned dependency; shared ESLint and Prettier come from the published `bottomlessmargaritas` config packages.
R-224: Dependencies flow one direction: higher layers import lower, never the reverse. Backend `handlers -> services -> repositories -> clients/db`; frontend `components -> hooks -> services/clients`. No upward imports, no layer skip that inverts flow, no circular imports between modules. Per-stack specifics in `CLAUDE-BACKEND.md` and `CLAUDE-FRONTEND.md`. Enforcement pending: `import/no-cycle` and `import/no-restricted-paths` in the shared `bottomlessmargaritas` ESLint config.
