...
R-220: No `lib/` or `utils/` directories. A service is a function-only module that performs business logic on inputs; `service` is the project term for what other stacks call helpers, utils, or lib. Place every such module under `services/`, grouped by responsibility (`services/llm/`, `services/db/`, `services/format/`). Name each subfolder for what lives in it.
...
R-222: Service directory layout. Name `services/` subfolders by responsibility along these axes: external integrations and IO by capability (`llm`, `db`, `email`, `pdf`, `file`, `content`, `auth`; `clients` for browser-side API wrappers); domain logic by domain, subdivided by operation (`jobs/match`, `jobs/generate`); provider-specific code centralized (every LLM call under `llm/`). One concern per folder. Co-locate non-code assets (fonts, fixtures) with the service that loads them. Keep export surface minimal: export only what is imported elsewhere; symbols used within one file stay unexported.
