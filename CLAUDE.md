...
R-233: Name variables descriptively; never abbreviate where the full word reads clearly, and optimize for readability over brevity. Avoid generic names (`data`, `value`, `result`, `temp`, `stuff`, `thing`, `helper`, `util`) unless the domain genuinely uses the term. A single value takes a singular noun; an array or collection takes a plural noun. Booleans follow R-232's `is`/`has`/`can`/`should` prefixes, never a bare adjective. A name must read as natural English when the code is read aloud; rename any name that does not communicate intent.
