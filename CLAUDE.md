...
R-224: Dependencies flow one direction: higher layers import lower, never the reverse. Backend `handlers -> services -> repositories -> clients/db`; frontend `components -> hooks -> services/clients`. No upward imports, no layer skip that inverts flow, no circular imports between modules. Per-stack specifics in `CLAUDE-BACKEND.md` and `CLAUDE-FRONTEND.md`. Enforcement pending: `import/no-cycle` and `import/no-restricted-paths` in the shared ESLint config (TypeScript), import-linter contracts (Python).
