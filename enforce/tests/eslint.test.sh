...
run "$TMP/services/types.ts" || { echo "FAIL: expected services/types.ts to be exempt from one-export (R-222)"; exit 1; }
run "$TMP/services/constants.ts" || { echo "FAIL: expected services/constants.ts to be exempt from one-export (R-222)"; exit 1; }
